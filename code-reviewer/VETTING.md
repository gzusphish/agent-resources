# Vetting Notes: code-reviewer

## Source
- **Repository:** alirezarezvani-claude-skills
- **Path:** `engineering-team/code-reviewer/`
- **Author:** Alireza Rezvani
- **License:** MIT

## Vetting Date
2026-03-31

## Purpose
Systematic code review with security, performance, and maintainability checks

## Approved For
- **Panopticon** (primary use case)
- **All projects** where applicable

## Testing Status
- **Skill validated:** Yes
- **Platform compatibility:** Windsurf, Claude Code

## Security Assessment
- **Risk level:** Low
- **External API calls:** None
- **File system operations:** As documented

## Installation
```bash
cp -r code-reviewer/ ~/.claude/skills/           # Claude Code
cp -r code-reviewer/ .windsurf/skills/          # Windsurf
```

## Notes
- Inducted from alirezarezvani-claude-skills
- Part of the agent-skills central repository
