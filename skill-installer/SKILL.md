...
description: "Use when deploying skills from the central agent-skills/ repository to IDE-specific contexts via symlinks. Handles installation, updates, rollbacks, and IDE-specific path management for Windsurf, Claude Code, GitHub Copilot, and Cursor."
---
...
- **Installing skills** from `agent-skills/` to a project via symlinks
...
**This skill DOES:**
- Create symlinks from `agent-skills/` to IDE contexts (single source of truth)
...
ln -s /absolute/path/to/agent-skills/skill-name target/.windsurf/skills/skill-name

# Windows Git Bash (use absolute paths with forward slashes)
ln -s "C:/Users/wyatt/git/agent-skills/skill-name" "target/.windsurf/skills/skill-name"
```
...
cp -r agent-skills/skill-name/ target/.windsurf/skills/
```
...
cp agent-skills/skill-name/SKILL.md target/.windsurf/skills/skill-name/
...
|-------|--------|------|---------|
| skill-name | agent-skills-custom/ | YYYY-MM-DD | Brief description |
```
...
diff -r agent-skills/skill-name/ target/.windsurf/skills/skill-name/
   ```
...
3. **Deploy update:**
   ...
   ln -s "~/.git/agent-skills/skill-name" "target/.windsurf/skills/skill-name"
   ```
...
git -C ~/.git/agent-skills/ checkout <previous-commit> -- skill-name/
ln -s "~/.git/agent-skills/skill-name" "target/.windsurf/skills/skill-name"
```
...
ls -la agent-skills/skill-a/  # source
...
mv agent-skills/skill-a agent-skills/skill-a-alt
ln -s "~/.git/agent-skills/skill-a-alt" "target/.windsurf/skills/skill-a-alt"
```
...
skill-vetter + skill-auditor (validate)
    ↓
CENTRAL REPO: agent-skills/ (unified custom + vetted)
    ↓
...
```bash
ln -s "~/.git/agent-skills/skill-name" "project/.windsurf/skills/skill-name"
```

**Install to multiple IDEs:**
```bash
for ide in .windsurf .claude .github; do
  ln -s "~/.git/agent-skills/skill-name" "project/$ide/skills/skill-name"
done
```
...
# Should show link to agent-skills/skill-name
```
...
cd ~/.git/agent-skills && git checkout <commit> -- skill-name/
ln -s "~/.git/agent-skills/skill-name" "project/.windsurf/skills/skill-name"
```
...
- `../agent-skills/` — Unified skill repository (all skills, custom and vetted)
