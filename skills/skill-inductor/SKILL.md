---
name: skill-inductor
description: "Use when discovering new skills from external sources, evaluating external skills for adoption, or ingesting approved skills into the unified agent-resources/skills repository. Guides the ingestion workflow from discovery through vetting. Does NOT handle IDE installation - use skill-installer for that."
---
...
- Placing vetted skills into `agent-resources/skills/` (unified repository)
...
- **Ingesting approved skills** into `agent-resources/skills/` (unified repository)
...
python agent-resources/skills/skill-vetter/scripts/vett.py /path/to/skill/ --strict

# JSON output for tracking
python agent-resources/skills/skill-vetter/scripts/vett.py /path/to/skill/ --json >> vetting-log.jsonl
```
...
```bash
# Copy to unified agent-resources/skills repository
cp -r /path/to/vetted/skill/ agent-resources/skills/
...
skill-inductor (THIS SKILL: discover → vet → ingest to central repo)
    ↓
CENTRAL REPO: agent-resources/skills/ (unified)
    ↓
...
3. Create VETTING.md
4. Copy to agent-resources/skills/
...
3. Review diff
4. Replace in agent-resources/skills/
...
| `agent-resources/skills/` | Unified repository (all skills, differentiated by VETTING.md source_type) |
